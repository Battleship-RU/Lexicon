<?php
/**
 * $Id: Request2Xml.php 40 2010-05-11 15:39:08Z dab $
 *
...
class HTTP_Request2Xml {
	 ...
	 */
	public function __construct( $schema, $request=NULL ) {
		...
		if( !$request &&
			...
			$this->dom->load("php://input"); //в документации указано неверно: поток называется input
			$this->empty=FALSE;
		} else {
			if( !$request ) {
							...
							if( $j<$len-1 ) {
								...
								if( is_array($val) ) {
									...
									if( array_key_exists($key, $_FILES) ) {
										...
										for( $a=0; $a<$count; $a++ ) {
											$children[$path]=$parent->appendChild($this->dom->createElementNS($ns,$name));
											$children[$path]->appendChild($this->dom->createElementNS($ns,"name",$this->cleanup($_FILES[$key]["name"][$a])));
	...
	protected function cleanup($value) {
		...
		//$replace_pairs["\xB9"]=N'; //русский "номер" заменить на N
		//$replace_pairs["&#8470;"]='N'; //русский "номер" заменить на N
		//$replace_pairs["Ё"]='Е'; //русскую Ё заменить на русскую E
		//$replace_pairs["ё"]='е'; //русскую ё заменить на русскую е
		//$replace_pairs["\xAB"]='"';//заменить ковычки << на "
		//$replace_pairs["\xBB"]='"';//заменить ковычки >> на "
		//$replace_pairs["&#171;"]='"';//заменить ковычки << на "
		//$replace_pairs["&#187;"]='"';//заменить ковычки >> на "
		//$replace_pairs["\x96"]='-';//заменить "длинное тире" на -
