...
abstract class Facebook_Base
{
  ...
  public function getLogoutUrl($params=array()) {
    return $this->getUrl(
      ...
      'logout.php',
      array_merge(array(
        'next' => Config::$destroy_path,//$this->getCurrentUrl(),
   ...
   * The cookie is not set by the Facebook_Base class, but
   ...
   * metadata. The cookie is not set by the Facebook_Base class, but it may be
  ...
  protected function makeRequest($url, $params, $ch=null) {
    ...
    if (curl_errno($ch) == 60) { // CURLE_SSL_CACERT
      ...
      curl_setopt($ch, CURLOPT_CAINFO,
                  dirname(__FILE__) . "/" . Config::$fb_ca_chain_bundle);
      $result = curl_exec($ch);
    }
    ...
    if ($result === false) {
      $e = new Facebook_ApiException(array(
  ...
  protected function throwAPIException($result) {
    $e = new Facebook_ApiException($result);
    switch ($e->getType()) {
  ...
  /**
   * Get the data for $key, persisted by Facebook_Base::setPersistentData()
   *
